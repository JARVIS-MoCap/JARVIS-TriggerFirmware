...
uint8_t SerialPeer::handleMessage(uint8_t *msg, size_t len)
{
    ...
    if (error_flags)
    {
        ...
        {
            error_len += snprintf((char *)error_str + error_len, SERIAL_PEER_MAX_BUFFER_SIZE - error_len, " # CRC ERROR / header.crc: %d / calc crc: %d", type_message->header.crc, crc);
        }
        if (error_flags & SERIAL_PEER_ERROR_LENGTH)
        {
            error_len += snprintf((char *)error_str + error_len, SERIAL_PEER_MAX_BUFFER_SIZE - error_len, " # LENGHT ERROR / header.length: %d / len: %d / MIN_LENGTH_MESSAGE: %d", length, len, MIN_LENGTH_MESSAGE);
        }
        if (error_flags & SERIAL_PEER_ERROR_NOT_IMPLEMENTED)
        {
            error_len += snprintf((char *)error_str + error_len, SERIAL_PEER_MAX_BUFFER_SIZE - error_len, " # NOT IMPLEMENTED ERROR ");
        }
        if (error_flags & SERIAL_PEER_ERROR_UNKNOWN_PACKET)
        {
            error_len += snprintf((char *)error_str + error_len, SERIAL_PEER_MAX_BUFFER_SIZE - error_len, " # UNKNOWN PACKET ERROR / header.type: %d", type_message->header.type);
        }
...
void SerialPeer::sendInputs(uint32_t uptime_us, uint32_t pulse_id, uint8_t inputs_state)
{
    ...
    msg->header.crc = calculateCrc((uint8_t *)msg + LENGTH_MSG_HEADER, msg->header.length);
